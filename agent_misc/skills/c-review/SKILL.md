...
Runs in the main conversation (invoke via `/c-review:c-review`). Orchestrator owns the `Task*` ledger as bookkeeping for retries; workers and judges have no Task tools. Workers and judges are named skill subagents (`c-review:c-review-worker`, `c-review:c-review-dedup-judge`, `c-review:c-review-fp-judge`); tool sets are declared in `agents/*.md`. Findings are exchanged via markdown-with-YAML files in a shared output directory.
...
**Path convention:** resolve `${C_REVIEW_ROOT}` to this skill's directory. Try `Bash: ls "${CLAUDE_PLUGIN_ROOT}/skills/c-review/prompts/clusters/buffer-write-sinks.md"` first, then fall back to `Bash: find ~/.claude -path '*/skills/c-review/prompts/clusters/buffer-write-sinks.md' -print -quit`, and finally `Bash: find . -path '*/skills/c-review/prompts/clusters/buffer-write-sinks.md' -print -quit`.
...
```bash
python3 "${C_REVIEW_ROOT}/scripts/build_run_plan.py" \
  --plugin-root "${C_REVIEW_ROOT}" --output-dir "${output_dir}" \
...
- `Agent(subagent_type="c-review:c-review-fp-judge", description="FP + severity judge", prompt=f"output_dir: {output_dir}\nsarif_generator_path: {sarif_generator_path}")` — resolve `sarif_generator_path` to `${C_REVIEW_ROOT}/scripts/generate_sarif.py`.
...
```bash
test -d "${output_dir}/findings" && python3 "${C_REVIEW_ROOT}/scripts/generate_sarif.py" "${output_dir}"
```
