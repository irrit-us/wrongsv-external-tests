...
import {
  ...
  type SetBreakpointInput,
  type AttachToProcessInput,
  type GetVariablesInput,
  type GetLocalVariablesInput,
  type GetStackTraceInput,
  type GetScopesInput,
  type EvaluateExpressionInput,
  type GetSourceContextInput,
  type StackFrame,
} from "./types.js";
...
const scopeObjectIds = new Map<string, number>();
let nextScopeRef = 1;

function assignScopeRef(sessionId: string, objectId: string): number {
...
function getObjectIdByRef(sessionId: string, ref: number): string | undefined {
...
function fmtFrame(f: StackFrame | undefined) {
  return f ? { name: f.name, file: f.file, line: f.line, column: f.column } : null;
}

function pauseInfo(sessionId: string, client: ReturnType<typeof sessionManager.getClient>) {
...
function fail(ctx: string, err: unknown) {
  return {
    content: [{ type: "text" as const, text: `Error ${ctx}: ${err instanceof Error ? err.message : String(err)}` }],
  };
}

// Workflow phase annotation used in _meta
type Phase = "setup" | "execution" | "inspection" | "teardown" | "discovery";
const wf = (phase: Phase, order: number) => ({ workflow: { phase, order } });
...
export function createServer(): McpServer {
  const server = new McpServer(
    {
      ...
      version: "1.0.0",
      description:
        "Interactive Node.js/JavaScript debugging via the V8 inspector protocol (CDP). " +
  ...
  server.registerTool(
    "create_debug_session",
    {
      ...
      description:
        "Create a new debug session for launching or attaching to a Node.js process. " +
    ...
    async (params: CreateDebugSessionInput) => {
      try {
  ...
  server.registerTool(
    "list_nvm_versions",
    {
      ...
      description:
        "List all Node.js versions installed via nvm, including paths and which version is the current default. " +
  ...
  server.registerTool(
    "list_supported_languages",
    {
      ...
      description:
        "List the languages and runtimes this debugger supports (Node.js/JavaScript). " +
  ...
  server.registerTool(
    "start_debugging",
    {
      ...
      description:
        "Launch a Node.js script under the V8 inspector and attach the debugger.\n\n" +
    ...
    async (params: StartDebuggingInput) => {
      try {
  ...
  server.registerTool(
    "attach_to_process",
    {
      ...
      description:
        "Attach the debugger to an already-running Node.js process via its V8 inspector port.\n\n" +
    ...
    async (params: AttachToProcessInput) => {
      try {
  ...
  server.registerTool(
    "detach_from_process",
    {
      ...
      description:
        "Detach the debugger from a running process without killing it. " +
  ...
  server.registerTool(
    "set_breakpoint",
    {
      ...
      description:
        "Set a breakpoint at a specific file and line number. When execution reaches that location the debugger pauses.\n\n" +
    ...
    async (params: SetBreakpointInput) => {
      try {
  ...
  server.registerTool(
    "step_over",
    {
      title: "Step Over",
      description:
        "Execute the current line without stepping into any function calls on it. " +
  ...
  server.registerTool(
    "step_into",
    {
      title: "Step Into",
      description:
        "Step into the function being called on the current line. If the line has no function call behaves like step_over.\n\n" +
  ...
  server.registerTool(
    "step_out",
    {
      title: "Step Out",
      description:
        "Resume execution until the current function returns, then pause at the call site.\n\n" +
  ...
  server.registerTool(
    "continue_execution",
    {
      ...
      description:
        "Resume script execution. The script runs until a breakpoint is hit, Debugger.pause is called, or the script completes.\n\n" +
  ...
  server.registerTool(
    "pause_execution",
    {
      ...
      description:
        "Interrupt a running script to inspect its state. Sends Debugger.pause via CDP to break into the debugger at the next opportunity.\n\n" +
  ...
  server.registerTool(
    "get_stack_trace",
    {
      ...
      description:
        "Return the current call stack while execution is paused. Frames are ordered innermost (index 0) to outermost. " +
    ...
    async (params: GetStackTraceInput) => {
      try {
        const client = sessionManager.getClient(params.sessionId);
  ...
  server.registerTool(
    "get_scopes",
    {
      title: "Get Scopes",
      description:
        "Return the scope chain for the current (topmost) stack frame. Scopes are ordered innermost to outermost. " +
    ...
    async (params: GetScopesInput) => {
      try {
        const client = sessionManager.getClient(params.sessionId);
        const frames = client.getCachedFrames();
        if (!frames.length) return ok({ error: "Not paused — no stack frames available" });
  ...
  server.registerTool(
    "get_variables",
    {
      title: "Get Variables",
      description:
        "Enumerate the properties of a scope or object identified by a variablesReference. " +
    ...
    async (params: GetVariablesInput) => {
      try {
        const client = sessionManager.getClient(params.sessionId);
        const objectId = getObjectIdByRef(params.sessionId, params.variablesReference);
        if (!objectId) return ok({ error: `No object for variablesReference ${params.variablesReference}. The session may have resumed.` });
  ...
  server.registerTool(
    "get_local_variables",
    {
      ...
      description:
        "Return all local variables in the topmost stack frame. Convenience wrapper that finds the local scope from the scope chain and enumerates it.\n\n" +
    ...
    async (params: GetLocalVariablesInput) => {
      try {
        const client = sessionManager.getClient(params.sessionId);
        const frames = client.getCachedFrames();
        if (!frames.length) return ok({ error: "Not paused — no stack frames available" });
        const localScope = frames[0].scopeChain.find(s => s.type === "local");
        if (!localScope) return ok({ error: "No local scope found in current frame" });
  ...
  server.registerTool(
    "evaluate_expression",
    {
      ...
      description:
        "Evaluate a JavaScript expression in the context of the current paused stack frame. " +
    ...
    async (params: EvaluateExpressionInput) => {
      try {
        const client = sessionManager.getClient(params.sessionId);
  ...
  server.registerTool(
    "get_source_context",
    {
      ...
      description:
        "Retrieve source code around a specific line in a debugged script. " +
    ...
    async (params: GetSourceContextInput) => {
      try {
        const client = sessionManager.getClient(params.sessionId);
        ...
        const ctx = [];
        for (let i = start; i < end; i++) ctx.push({ line: i, content: lines[i], current: i === params.line });
  ...
  server.registerTool(
    "list_debug_sessions",
    {
      ...
      description:
        "List all active debug sessions with state, Node version, script path, and breakpoint info. " +
  ...
  server.registerTool(
    "list_threads",
    {
      title: "List Threads",
      description:
        "List execution threads for a debug session. Node.js uses a single-threaded event loop, " +
  ...
  server.registerTool(
    "close_debug_session",
    {
      ...
      description:
        "Terminate a debug session and the associated Node.js process. " +
