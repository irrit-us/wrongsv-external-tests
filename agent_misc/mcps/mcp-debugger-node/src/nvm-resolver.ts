...
function getNvmDir(): string {
  if (existsSync(VERSIONS_DIR)) return VERSIONS_DIR;
  throw new Error(
    `nvm versions directory not found at ${VERSIONS_DIR}. Is nvm installed?`
  );
}
...
function findClosestVersion(target: string): string {
  const versions = listInstalledVersions();
  ...
  throw new Error(
    `No Node.js version matching '${target}' found. Installed: ${versions.join(", ")}`
  );
}

export function resolveNodeVersion(version: string): NvmVersion {
...
export function listNvmVersions(): NvmVersion[] {
  const versionsDir = getNvmDir();
  const versions = listInstalledVersions();
...
export function getCurrentNvmVersion(): string {
  try {
    const out = execSync("nvm current", {
      encoding: "utf-8",
      shell: "/bin/bash",
