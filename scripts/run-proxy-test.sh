...
" 2>/dev/null) || CONNECT_STATUS="unknown"
  ...
  # For Hiddify, the proxy port is auto-generated by the core and
  # stored in current-config.json. Detect it after connection.
  if [[ "$APP" == "hiddify" ]]; then
    HIDDIFY_DATA_DIR="${DATA_DIR:-$HOME/.local/share/hiddify}"
    HIDDIFY_CONFIG="$HIDDIFY_DATA_DIR/data/current-config.json"
    if [[ -f "$HIDDIFY_CONFIG" ]]; then
      REAL_PORT=$(python3 -c "
import json
with open('$HIDDIFY_CONFIG') as f:
    cfg = json.load(f)
for inbound in cfg.get('inbounds', []):
    if inbound.get('type') in ('mixed', 'http', 'socks'):
        print(inbound.get('listen_port', ''))
        break
" 2>/dev/null)
      if [[ -n "$REAL_PORT" ]]; then
        echo "  Real proxy port (from Hiddify config): $REAL_PORT"
        PROXY_PORT="$REAL_PORT"
        # Update the status file with the real port
        sed -i "s/PROXY_PORT=.*/PROXY_PORT=$PROXY_PORT/" /tmp/${APP}_status.env 2>/dev/null || true
        sed -i "s|SOCKS5_PROXY=.*|SOCKS5_PROXY=socks5://127.0.0.1:$PROXY_PORT|" /tmp/${APP}_status.env 2>/dev/null || true
        sed -i "s|HTTP_PROXY=.*|HTTP_PROXY=http://127.0.0.1:$PROXY_PORT|" /tmp/${APP}_status.env 2>/dev/null || true
      fi
    fi
  fi
